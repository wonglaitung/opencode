#!/usr/bin/env bash
#
# pack-bundle.sh —— 把 opencode-server-debug 打成可移植的整包便携 tarball。
#
# 原理：
...
#   4. 打包为 dist/opencode-server-debug-bundle-<version>.tgz。
#
# 与 opencode-edge-debug 的 pack:bundle 对齐，但本工程是单插件：
# 无 workspace 包、无 CLI、无收集服务，故省略「补齐 workspace 包」步骤。
# 校验项改为检查 ssh 客户端（而非 Edge）。
#
# 用法：
...
#
# 目标机使用：
#   tar xzf opencode-server-debug-bundle-<version>.tgz
#   cd opencode-server-debug-bundle-<version>
...
bundle_name="opencode-server-debug-bundle-${version}"
...
# opencode-server-debug-bundle ${version}

OpenCode 按需远程服务器日志调试插件（整包便携版）。
...
> \`setup.sh\` / \`setup.ps1\` 只是**可选的环境自检脚本**，仅打印检查项、不安装任何东西。
> 不解压后直接把 \`opencode.json\` 的 \`plugin\` 指向解压目录即可使用；只有想预先确认 bun / ssh 客户端是否就绪时才运行它。
