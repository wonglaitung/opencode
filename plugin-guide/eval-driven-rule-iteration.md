...
| 行为面 | 本方法论（评测门 / 质量飞轮） | 规则文本 / 探针清单 / 打分卡 / 模板 / 工具描述 / 状态条 / 注入格式——**模型读进上下文的内容** |

**行为面改进落点 = 注入模型上下文的内容，具体四类**（质量飞轮每一轮改的就是这些，不是机制面代码）：

1. **规则文本**（`WorkflowDefinition.rules`，按阶段注入）——如 reqdoc-r11 探针清单、r21 打分卡、r22 探针门禁、r23 渲染校验、r24 渲染门禁；
2. **工具描述**（`reqdoc_probe` / `reqdoc_check` / `reqdoc_score` / `reqdoc_confirm_features` 等的 `description`）——模型选工具时读的说明；
3. **状态条**（如「追问覆盖」「渲染校验」行）——随会话注入的一行式反馈；
...
**注意区分「改进落点」与「评测判定口径」**：`scenarios.ts` / `judge.ts` / `run.ts` 等**评测脚本不属于行为面**——它们是对照物/量尺：飞轮改的是上面四类注入内容，用判定口径去量改的效果（通过率 / 五维分）。判定口径改动同样要过评测门（判据变了、结果不可直接对比），但那是「换尺子」不是「改被测内容」。行为面改动分级决策图见下。

**改动分级**（哪些改动才需要跑评测门）：

```mermaid
flowchart LR
    A(["改动"]) --> B{"碰触模型行为面？<br/>（模型读进上下文的内容：<br/>规则 / 探针 / 打分卡 / 模板 / 状态条<br/>工具描述 / 注入格式；<br/>以及评测脚本=换尺子也须过评测门）"}
