#!/usr/bin/env bash
#
# sync-bundle.sh —— 从本机（WSL/Linux）把 edge-debug 插件源码改动镜像进 Windows 上已解压的插件目录，
# 免去手工 copy。
#
# 与 opencode-session-mgmt 的 sync-bundle.sh 同型，但本工程是单插件（无 workspace 包双份）：
# 插件以 package.json 的 main=src/index.ts 被 OpenCode 直接加载，整份源码就是运行所需。
#
# 同步范围：镜像 src / test / package.json / README.md / CLAUDE.md / bun.lock / .npmrc。
...
#   BUNDLE="/mnt/c/其它路径/opencode-edge-debug-bundle-0.0.1" bash scripts/sync-bundle.sh
#
# 改完请重启 Windows 上的 opencode 守护进程（关闭再开）让插件重新加载。
#
set -euo pipefail

# 脚本位于 opencode-edge-debug/scripts/，切到插件根
cd "$(dirname "$0")/.."
REPO="$(pwd)"

# 默认指向 Windows 测试 bundle（WSL 下 C:\ 挂在 /mnt/c）
BUNDLE="${BUNDLE:-/mnt/c/Users/User/Documents/My Tools/node-v22.23.2-win-x64/opencode-edge-debug-bundle-0.0.1}"
