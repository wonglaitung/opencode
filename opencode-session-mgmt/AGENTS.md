...
- **reqdoc 重构（双通道 + 功能点拆解 + PRD 质量打分卡，目标：辅助业务写需求）**：业务「口述 + 丢材料」，AI 代笔。目录契约改 00~07（编号即五步编写流顺序：00_初稿需求书 为初稿导入入口；01_背景与目标 / 02_流程与数据 / 03_制度与合规 / 04_角色与权限 为业务投放材料区；05_系统现状与能力 为可选目录；06_功能点、07_需求规格产出为 AI 工作区）。**文档扫描经专用工具 `reqdoc_scan(directory)`**（单目录参数、按阶段分步调用，解析 docx/pdf/xlsx/txt/md/json/csv；qwen3.6 纯文本无多模态，图像显式降级提示文字描述）。**prd 前置功能点拆解**：综合材料 + 问答拆功能点清单 → 业务确认后 `reqdoc_confirm_features(features)` 记录并建 `06_功能点/N_名称/` 子目录（来源摘录），同时幂等预建 `07_需求规格产出/N_名称/`。**PRD 质量打分卡（实施方案第三节）**：edge 完成进 prd 前经 `reqdoc_score` 八维打分（满分 100，`total` 服务端计算，`business_confirmed` 强制）→ 按《业务需求说明书》模板渲染，逐字段标来源 `[文档]/[问答]/[缺省]`，绝不杜撰事实（`[问答]` 口语须提炼为规范书面语、不得原话照搬对话、不得为填字段虚构未确认内容），渲染严格逐字遵循模板（reqdoc-r20 铁律，扣分项按字段映射落位）；≥85 分且业务确认才可进入 prd 渲染与定稿（workflow_advance enter(prd) + review_submit 两处硬门禁，仅 reqdoc）；未达标按三档分级引导（<60 补主流程与异常、60-84 补脱敏/权限/逆向、≥85 停止追问），追问严禁纯技术词汇（reqdoc-r21/r2，实施方案第四节）。展示得分附质量得分进度条（reqdoc_score 返回文本服务端渲染 10 格）。扣分明细含证据仅本机留痕、汇报不上行。PRD 定稿后经 `reqdoc_export(source=md路径)` 导出 Word 交付件（md→docx，docx 库，与 md 同目录归档）。**三大支柱与加固优先级**：追问（喂入）→ 打分（门禁）→ 模板（交付）三根支柱缺一不可；模板/打分已结构性固化（模板权威源 + 单一事实源 + 服务端算分，漂移风险低）。**追问已升级为结构化探针清单（质量飞轮 P1）**：`REQDOC_PROBES` 7 条探针（与打分卡维度一一映射，`reqdocProbeRubric()` 单点事实源注入 reqdoc-r11 规则文本，`reqdoc_probe` 工具描述只引用不复述——P3 上下文瘦身），每轮追问结束调用 `reqdoc_probe(asked, gaps, round)` 记录进 `WorkflowState.probes`；**P0.1 探针记录强制前置**——进入 prd 前必须已调用过 `reqdoc_probe`（workflow_advance enter(prd) 未记录即拦截，材料全覆盖可记录一次），缺口一致校验门禁只拦「缺口+满分矛盾」（`probeGapViolations`，workflow_advance 进 prd 与 review_submit 两处），状态条展示探针覆盖，评测经 `argsContains` 断言。**P2.5 字段定义（数据字典）硬门禁**：进 prd 渲染前逐字段与业务确认并调用 `reqdoc_field_dict` 生成数据字典（写入 `07_需求规格产出/数据字典与库表设计/数据字典.md`），`workflow_advance enter(prd)` 与 `review_submit` 两处强制 `fieldDict` 非空（需求确无结构化字段可 `skip_field_dict=true` 豁免，两处工具均有该参数）。**渲染已升级为结构校验（质量飞轮 P2，渲染可测化）**：模板抽象为结构 schema（`REQDOC_TEMPLATE_CHAPTERS`/`REQDOC_TEMPLATE_FIELDS`，r20 字段映射结构化，`renderCheckRubric()` 单点事实源注入 reqdoc-r23 规则文本，`reqdoc_check` 工具描述只引用不复述），渲染完成后调 `reqdoc_check(source)` 对照 schema 做渲染 diff 校验（章节骨架/功能点块数/映射字段来源标注，`parseRenderStructure` 运行时工具与评测 render 判定同源），写入 `WorkflowState.render`；柔性门禁（用户定：柔性 + 定稿复核）——未记录放行，一旦记录 review_submit 定稿时重读源 md 复核（`renderStructureViolations`/`renderGapViolations`，结构违规与「[缺省]↔满分」矛盾拦截）。**渲染机制（P1/P2/P3 小模型友好改造）**：PRD 骨架由服务端 `reqdoc_render_skeleton(source)` 按模板逐字生成（第 1~4、6、7 章取模板正文，第五章按已确认功能点生成空块），消除模型巨型 write 截断；内容经 `reqdoc_patch(source, target, content)` 按小节编号逐段填充（服务端 `patchSectionBody` 定位标题、保证编号，模型只产出内容）；模板全文不再注入对话，改为注入结构摘要 `renderTargetDigest()`（约 1k 字符，替代原 7.6k），且 `reqdoc_check`/`reqdoc_score`/`reqdoc_probe` 工具描述不再内嵌评分/探针/结构 rubric（单点事实源在 r11/r21/r23 规则文本）。旧 `reqdoc_extract_feature`/`reqdoc_replace_feature` 已移除。sdlc 完全不动。
