...
export const SCENARIOS: Scenario[] = [
  ...
  // 解析（与运行时 reqdoc_check 同源）。与 r18/r19 的 score 判定互补：score 抓八维质量、render 抓结构。
  {
    ...
    userTurn:
      "材料齐全，按《业务需求说明书》模板渲染：第一章 项目信息（项目名称/编号）；第二章 文档变更过程；第三章 需求概述（3.1 需求类型 流程优化 [文档]、3.2 属于流程优化项目、3.3 涉及跨部门项目、3.4 涉及总行开发、3.5 希望完成时间、3.6 需求提出原因及功能概述）；第四章 术语定义与业务规则（4.1 术语定义、4.2 业务规则）；第五章 需求功能详述。功能点 1 柜台跨行转账：输入要素 1.1 简要概述 [文档] 柜员发起转账、1.2 控制要求 [文档] 留痕与双人复核；处理要求 2.1 输入要素的检查 [文档] 校验卡号余额、2.2 系统处理过程 [文档]、2.3 异常处理要求 [问答] 网络超时冲正、2.4 提示信息 [文档]、2.5 其他要求 [缺省]、2.6 清算处理 [文档] 次日清算、2.7 差错处理 [文档] 失败可冲正、2.8 交易安全性 [文档] 手机号脱敏、2.9 数据存贮和清理 [文档] 留档 5 年、2.10 附件。功能点 2 转账进度查询：输入要素 3.1 [文档] 客户查询进度、3.2 控制要求 [问答] 仅本人可查；处理要求 4.1 输入要素的检查 [文档] 凭交易号查询、4.2 系统处理过程 [文档]、4.3 异常处理要求 [文档] 查询超时重试、4.4 提示信息 [文档]、4.5 其他要求 [缺省]、4.6 清算处理 [缺省]、4.7 差错处理 [文档]、4.8 交易安全性 [文档] 遮罩展示、4.9 数据存贮和清理 [文档]、4.10 附件。逐字段标来源 [文档]/[问答]/[缺省]。",
    // 渲染 diff 判定：五章齐全且顺序正确、2 个功能点块、映射字段逐功能点全标来源
    // soft（A3/D7 拆级）：来源标注降为观察项不计通过率——硬门禁只留结构骨架（章节/顺序/块数）
    judge: {
      kind: "render",
      requiredChapters: ["第一章 项目信息", "第二章 文档变更过程", "第三章 需求概述", "第四章 术语定义与业务规则", "第五章 需求功能详述"],
      ordered: true,
      minFeatures: 2,
      sourceAll: true,
      fuzzy: true,
