...
| [模版.docx](模版.docx) | 运行时资源 | reqdoc PRD 模板的 **冻结源**（内容以本文件为准，禁止改） | [pack-bundle.sh](../scripts/pack-bundle.sh)（整目录拷入 bundle）、[session-management.md](session-management.md)、[reqdoc-prd-template.md](reqdoc-prd-template.md) 头部标注 | 可改名（同步 3 处引用）；**内容冻结，改 docx 必须同步重渲染 md** |
...
- **[reqdoc-prd-template.md](reqdoc-prd-template.md) 与 [模版.docx](模版.docx) 是同一模板的两个形态**：docx 为冻结源、md 为运行时载体。
...
- **[reqdoc-prd-template.md](reqdoc-prd-template.md) 的位置是代码契约**：[template.ts](../packages/plugin/src/template.ts) 按 `packages/plugin/src` 上溯三级解析到
  ...
  「模板送达」机制，**不要动**。
- **[模版.docx](模版.docx) 内容冻结**：冻结的是内容而非文件名；若要改名，同步 [pack-bundle.sh](../scripts/pack-bundle.sh) 注释、
