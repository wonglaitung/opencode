...
| reqdoc-r14 | prd | 按《业务需求说明书》模板**分段渲染**最终 PRD（模板为权威源，逐字落实由服务端 `reqdoc_render_skeleton` 生成骨架保证；系统提示「渲染目标结构」段给出结构摘要，见 r20）。三步：① `reqdoc_render_skeleton(source)` 生成骨架（章节 1~7，第五章含已确认功能点空块，勿用 write 手写整篇）；② `reqdoc_patch(source, target, content)` 逐小节填充（第三章 3.1~3.6、第四章 4.1/4.2、第六章 6.1~6.4、第七章 7.1/7.2，及每功能点 5.k.1.1~5.k.2.13；每次 1~3 小节，内容取自 06_功能点/N_名称/ 来源摘录 + 问答），逐字段标来源 [文档]/[问答]/[缺省：不适用理由]，[问答] 口语须提炼为规范书面语、绝不杜撰；③ `reqdoc_check(source)` 全篇校验，违规用 reqdoc_patch 修正。产出归档：澄清记录、数据字典与库表设计、RBAC 权限矩阵与审批流、最终 PRD 写入 07_需求规格产出；定稿后 `reqdoc_export(source)` 导出 Word 交付件。 |
