...
1. **不是「一直跑」，是「跑 → 看归因 → 改一处 → 再跑」**：每次迭代跑出基线 → 归因出「哪一维弱、对应哪根支柱」→ 人改对应探针 / 打分卡 / 模板 / 规则 → 再跑确认那维回升、其它维不降。人始终在环里，飞轮只是指方向的数据源（六步闭环见 `plugin-guide/eval-driven-rule-iteration.md` 2 章）。
...
准确说法：「跑飞轮 + 按归因人工改提示词 + 多模型复验」的循环，能迭代出一套在打分卡五维与评测场景集覆盖范围内、适配多模型的提示词，用于协助业务写需求——**人机协同的迭代**，不是全自动优化。
