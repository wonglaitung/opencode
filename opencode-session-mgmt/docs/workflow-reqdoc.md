...
| reqdoc-r27 | global | 关键确认防浅背书：若业务连续 2 次选择「默认推荐 / 同意默认」，须在当轮显式要求业务对 3 项关键内容给出自主意见——量化目标（如 MTTR 基线/目标具体数字，补满 100 分）、功能范围、权限边界——不得继续走默认；量化目标缺失时直接追问具体数字。目的在防止「AI 揣测 + 业务走流程」式假确认。此处不做硬拦截，靠 reqdoc-r21 打分门禁兜底。 |
