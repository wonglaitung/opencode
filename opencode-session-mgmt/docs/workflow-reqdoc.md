...
| reqdoc-r14 | prd | 按《业务需求说明书》模板渲染最终 PRD（模板 `docs/reqdoc-prd-template.md`；模板全文由插件在 prd 阶段自动注入系统提示（见「模板全文」段），以注入的模板全文为唯一依据，渲染须严格逐字遵循（见 r20），插件找不到模板文件时才按内联骨架渲染）：封面（项目信息表、文档变更过程表）→ 第三章 需求概述（需求类型/流程优化/跨部门/总行开发/希望完成时间/提出原因及功能概述）→ 第四章 术语定义与业务规则（术语定义、业务规则）→ 第五章 需求功能详述（按已确认功能点：输入要素/处理要求/异常/清算/差错/交易安全/数据存贮/附件；功能点 k 编号全局连续——输入要素为第 (2k-1) 组、处理要求为第 (2k) 组）→ 第六章 非功能需求（性能与容量/可用性与可靠性/安全与信创/数据主权与合规）→ 第七章 验收标准（功能点验收指标/量化验收口径）。每功能点内容从 06_功能点/N_名称/ 来源摘录 + 问答补全，逐字段标来源 [文档]/[问答]/[缺省]，绝不杜撰事实；[问答] 来源的口语须提炼整理为规范的需求书面语（去除闲聊、口头禅与不完整表述、保留业务原意），不得原话照搬对话文字，但不得为填满字段虚构业务未确认内容；未涉及项选「不涉及/不适用」并留白；项目信息表与文档变更过程属项目元数据，不主动问业务，渲染时留空占位。产出归档：澄清记录、Mermaid 流程图、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑、最终 PRD 写入 07_需求规格产出；PRD 定稿后调用 reqdoc_export(source=PRD 路径) 生成 Word 版（.docx）交付件，与 md 同目录归档。 |
...
| reqdoc-r23 | prd | 渲染结构校验：PRD 渲染完成并写入 07_需求规格产出 后，调用 reqdoc_check(source=PRD md 路径) 对照模板结构 schema（章节骨架齐全且顺序正确、功能点块数与已确认功能点一致、打分卡映射字段逐功能点标来源；标准经 `renderCheckRubric()` 同源注入工具描述与规则文本，见 7 章）做渲染 diff 校验。校验有违规（缺章节/乱序/块数不符/漏来源）须修正渲染后重调复查；结构合规后再 review_submit 定稿（定稿时重读源 md 复核）。 |
