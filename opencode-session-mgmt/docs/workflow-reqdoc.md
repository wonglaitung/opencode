...
│   └── README.md
├── 07_系统现状与能力/      # 【单位技术侧投放，可选】系统架构/现有能力/对外接口/数据模型；reqdoc_scan 按需拉取（不由 init 自动建，单位按需自建）
└── 需求资料目录说明.md      # 初始化自动生成：七目录总览（含可选 07_系统现状与能力；独立文件名，避免覆盖业务自有 README.md）
```
...
| `07_系统现状与能力` | edge（边界与异常） | 系统架构/现有能力/接口/数据模型提取，作为 prd 撰写依据 | 技术侧确认系统能力边界 |

`01/03` 喂「内容」阶段（goal/rules），`02/04` 喂「探针」阶段（edge），`05` 是功能点拆解区，`06` 是产出端——与 reqdoc 五阶段天然对齐。

**知识层约定（可选目录 07）**：系统架构/现有能力/接口/数据模型与具体需求发起单位强相关，**不内置进插件，也不依赖业务不可维护的 AGENTS.md**；由单位技术侧投放至 `07_系统现状与能力/`，Agent 经既有通用工具 `reqdoc_scan(directory="07_系统现状与能力")` 消费。相应规约 `07-系统上下文与现有能力.md`（stage: edge）强制「先扫后写、严禁杜撰」，直接支撑打分卡 `material` / `flowClosure` / `businessValue` 维度。该目录不由 `reqdoc_init` 自动生成，单位按需自建。
