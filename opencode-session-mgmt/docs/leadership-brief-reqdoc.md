...
```mermaid
graph LR
    IMP["已有初稿（docx/pdf/txt）"] --> IMP2["reqdoc_import 落盘 00_初稿需求书<br/>+ 规约初评"]
    IMP2 -. 诊断后逐步补全（不自动快进） .-> G
    G[目标与场景] --> R[流程与规则]
    R --> E[边界与异常]
    E --> P[需求规格书]
    P --> V[业务确认]
    V --> Gate{定稿门禁<br/>完整性（裸[缺省]拦截）<br/>一致性（更改须点明改造）<br/>来源真实性}
...
```mermaid
graph TD
    IMP["已有初稿"] --> IMP2["reqdoc_import + 规约初评"]
    IMP2 -. 逐阶段补全 .-> Q
    I[业务口述 + 投材料] --> Q[① 引导追问]
    Q --> S[② 打分卡 ≥85]
    S --> T[③ 模板渲染]
...
业务若已有零散初稿（docx / pdf / txt 等），可直接丢给系统：reqdoc_import 把初稿落盘 00_初稿需求书、并据这 7 份规约给出「规约初评」（逐条满足 / 缺失 / 矛盾 + 三类补全路径——AI 改、补材料、对话补），**不自动代写、不快进**，先诊断再逐步补全；符合 IEEE 29148 与银行业 BRD 的结构化要求。
...
成稿严格逐字遵循行方《业务需求说明书》模板，从封面、术语、到每功能点的输入 / 处理 / 异常 / 安全性 / 数据存贮。综合材料 + 问答拆成功能点清单（编号 / 名称 / 优先级），业务确认后逐字段定义名称 / 类型 / 长度 / 必填 / 取值 / 来源系统，形成数据字典；逐字段标来源——文档提取标 `[文档]`、问答补全标 `[问答]`、尚未获得标 `[缺省]`，绝不杜撰；**每功能点新增「接口与数据源」「权限与最小授权」两个必填子项（对应 2.11 / 2.12，落实最小授权与接口可溯源）**，且 `[缺省]` 须带「不适用理由」，裸 `[缺省]` 在定稿复核拦截；「可验证与验收」规约要求每条需求附功能 / 非功能 / 监管合规三类可测验收标准；定稿后自动在「文档变更过程」维护版本记录（初始定稿 1.0，重做修订 1.N），重做轨迹可追溯。
...
| acceptability | 可验收性 | 7 | 每功能点须有量化验收指标 |
  ...
  - **完整性门禁**：模板来源标注须带理由，裸 `[缺省]`（无「不适用理由」）在定稿复核拦截，杜绝「占位不填」。
  - **一致性门禁**：需求类型选「更改功能」时，须于概述点明对现有功能/系统的改造对象与变更边界，否则定稿拦截——防止「改旧系统却写得像从零新建」。
...
| 能力 | 状态 |
|---|---|
| 需求编写五步流程 | 已落地；定稿前强制业务确认，不可跳过，可反复迭代 |
| 机构规约按需注入 | 已落地；零代码扩展，已内置 7 份 reqdoc 规约 |
| 引导式追问探针 | 已落地；7 条探针清单，进规格书前强制记录覆盖、缺口与打分一致校验 |
| PRD 质量打分卡 | 已落地；8 维 100 分、≥85 硬门禁、业务确认强制、两处硬拦截 |
| 功能点拆解 | 已落地；综合材料 + 问答拆功能点清单，业务确认后建子目录 |
| 字段定义（数据字典） | 已落地；进规格书前逐字段与业务确认，硬门禁 |
| 资料扫描 | 已落地；reqdoc_scan 解析 docx / pdf / xlsx / txt / md / json / csv |
| 来源真实性门禁 | 已落地；[文档] 占比 ≥30% 或 ≥2 功能点含文档支撑，否则定稿拦截 |
| 模板化 Word 交付件 | 已落地；reqdoc_export 生成 .docx 与 md 同目录归档 |
| 接口与数据源 / 权限最小授权 | 已落地；每功能点 2.11 接口与数据源、2.12 权限与最小授权为必填子项，最小授权可溯源 |
| 完整性门禁（裸 [缺省] 拦截） | 已落地；模板来源标注须带理由，裸 [缺省] 定稿复核拦截 |
| 一致性门禁 | 已落地；选「更改功能」须于 3.6 点明改造对象，否则定稿拦截 |
| 基于初稿完善 | 已落地；reqdoc_import 把业务已有初稿落盘 00_初稿需求书，不自动快进 |
| 规约初评 | 已落地；reqdoc_review_conventions 按 7 份规约对初稿逐条点评，给三类补全路径 |
| 变更过程版本记录 | 已落地；定稿自动写「文档变更过程」表，初始 1.0、重做 1.N |

## 六、兼容现状：不改变业务现有习惯

- 业务「口述 + 丢材料」完全 OK：材料放进 01~05 目录 AI 扫描提取，没材料就口述补全，无需一次备齐；
