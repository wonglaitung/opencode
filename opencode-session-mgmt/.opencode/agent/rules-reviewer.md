...
1. **已定案清单**：对照 CLAUDE.md「已定案，勿重议」逐条检查（身份全手填、插件不读上游数据库、group 为名称字符串、收集服务三端点、CLI 命名 opencode-sm、完成门禁模型、规则阶段化注入、comprehension_confirm 单段、applyTransition 状态机语义等），不得引入与已定案冲突的设计。
...
4. **文档同步**：行为变更必须同步设计文档族对应章节及其 mermaid 流程图（三个文档合计 19 个：session-management.md 16 个、workflow-reqdoc.md 3 个、workflow-sdlc.md 0 个；改链路必改图）；注释/文档禁止使用 `§` 符号。
