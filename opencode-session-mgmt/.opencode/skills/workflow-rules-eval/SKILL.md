...
description: Use when running or analyzing the rule-following evaluation baseline (eval-rules, 规则遵循度评测, 13.1) in the opencode-session-mgmt project — how well weak models follow the injected workflow rules (pass-rate, sdlc + reqdoc) plus the reqdoc 质量飞轮 five-dimension PRD score iteration (46 scenarios, dual-model baseline freeze, --variant new 五维 delta, attribution to 追问探针/打分卡/模板, convergence) and scorecard changes (改评分标准 → 评测迭代 → 重冻结基线).
---
...
4. **基线状态确认**——首次改前先冻结 46 场景双模型基线（当前未冻结）；已冻结则只跑 new、不重跑 baseline（→「质量飞轮·现状 / 冻结基线」）
...
- `results/baseline.json`、`results/new.json`：运行结果，`new` 自动对比 `baseline`；多模型跑互覆盖，用 `cp` 保存第二份（如 `new-<model>.json`）
...
`results/` 里当前是**旧 31 场景快照**（无五维分数：`baseline.json` 14 场景 / `new*.json` 31 场景）。46 场景双模型基线**未冻结**——五维 delta 要等新 baseline 带 score 快照后才打印（run.ts 仅 baseline 与 new 都有 `summary.score` 才输出逐维对比）。**开发 reqdoc 的第一件事：冻结新基线。**

### 冻结 46 场景双模型基线（一次性，之后别重跑）
...
# ② 远端推理模型冻结（五维分数的基准参照）
EVAL_BASE_URL=https://<端点>/v1 EVAL_API_KEY=<key> EVAL_MODEL=<model> EVAL_MAX_TOKENS=4096 \
...
冻结结果 `baseline.json` **入库 commit** 作参照。日常改动只跑 `--variant new`，不要重跑 baseline 覆盖参照，否则对比失效。
...
3. **③ 真实模型回归**（重闸）：主模型 + 弱模型各一遍，双模型结果分开保存。
   ```
   EVAL_BASE_URL=... EVAL_MODEL=<主模型> EVAL_MAX_TOKENS=4096 \
     bun run scripts/eval-rules/run.ts --variant new --repeat 3
   cp scripts/eval-rules/results/new.json scripts/eval-rules/results/new-<主模型>.json
   EVAL_BASE_URL=http://localhost:8086/v1 EVAL_MODEL=/models/qwen3 EVAL_MAX_TOKENS=2048 \
     bun run scripts/eval-rules/run.ts --variant new --repeat 3
   ```
