...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    // ---- prd 需求规格书 ----
    { id: "reqdoc-r13", stage: "prd", text: "将对话信息自动渲染成《业务需求说明书》模板（见 docs/reqdoc-prd-template.md）：封面（项目信息、文档变更过程）→ 第一章 需求概述（需求类型/流程优化/跨部门/总行开发/希望完成时间/提出原因及功能概述）→ 第二章 需求概述（术语定义/业务规则）→ 第三章 需求功能详述（逐功能点：输入要素、处理要求、异常/提示/清算/差错/交易安全/数据存贮/附件）；未涉及项在 ○/● 中选「不涉及/不适用」并留白正文；业务说资料已放好则扫描 06 目录作输入。" },
