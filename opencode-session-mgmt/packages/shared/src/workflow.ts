...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r14", stage: "prd", text: "按《业务需求说明书》模板渲染最终 PRD（模板：docs/reqdoc-prd-template.md；模板全文由插件在 prd 阶段自动注入系统提示，见「模板全文」段；以注入的模板全文为唯一依据，渲染须严格逐字遵循，见 reqdoc-r20；仅当插件找不到模板文件时才按内联骨架渲染）：封面（项目信息表、文档变更过程表）→ 第三章 需求概述（需求类型：新增/更改、流程优化/跨部门/总行开发、希望完成时间、提出原因及功能概述）→ 第四章 术语定义与业务规则（术语定义、业务规则）→ 第五章 需求功能详述（按已确认功能点：编号/名称/优先级，输入要素：简要概述/控制要求，处理要求：输入要素检查/系统处理过程/异常处理/提示信息/其他要求/清算处理/差错处理/交易安全性/数据存贮和清理/附件；功能点 k 编号全局连续——输入要素为第 (2k-1) 组、处理要求为第 (2k) 组，功能点 1 用 1./2.、功能点 2 用 3./4.、功能点 N 用 (2N-1)./(2N).，各功能点下不再重复 1.1）。每功能点内容从 05_功能点/N_名称/ 子目录的来源摘录 + 问答补全，逐字段标来源：文档提取标 [文档]、问答补全标 [问答]、尚未获得的信息留白并标 [缺省]，绝不杜撰事实；[问答] 来源的口语须提炼整理为规范的需求书面语（去除闲聊、口头禅与不完整表述、保留业务原意），不得原话照搬对话文字，但不得为填满字段而虚构业务未确认的内容；未涉及项在 ○/● 中选「不涉及/不适用」并留白正文；项目信息表与文档变更过程属项目元数据，不主动问业务，渲染时留空占位。产出归档：需求澄清记录、自动提取的 Mermaid 流程图、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑、最终 PRD 一律写入 06_需求规格产出 目录；PRD 定稿后调用 reqdoc_export(source=PRD 路径) 生成 Word 版（.docx）交付件，与 md 同目录归档。" },
