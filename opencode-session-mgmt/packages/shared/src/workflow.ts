...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r30", stage: "global", text: "来源真实性门禁（防全[问答]兜底）：PRD 定稿须有一定书面材料支撑——[文档] 来源占比 ≥30%，或至少 2 个功能点含 ≥1 处 [文档] 支撑，二者满足其一即可；均不满足则 review_submit 被拦截，须业务向 01~05 补充书面材料后重扫 reqdoc_scan，或由业务明确确认「无书面材料可引用」(no_document_confirmed=true) 后定稿。" },
    ...
    { id: "reqdoc-r8", stage: "goal", text: "目录就绪检查：项目根约定 00~07 需求资料目录（编号即五步编写流顺序）：00_初稿需求书 为已有初稿导入入口；01_背景与目标、02_流程与数据、03_制度与合规、04_角色与权限 为业务投放材料区；05_系统现状与能力 为可选目录（单位技术侧投放系统架构/现有能力/接口/数据模型）；06_功能点、07_需求规格产出为 AI 工作区。尚无时主动调用 reqdoc_init 搭建骨架（幂等，绝不重建或覆盖业务已放材料），并向业务展示各材料目录的绝对路径、明确说明「把资料放进 01~05 对应目录，有多少投多少，未投放的目录我们口述补全，无需一次备齐」；业务已有初稿则放进 00_初稿需求书/ 后调 reqdoc_import(path) 导入。业务说资料已放好、或会话中途补充了材料，则调用 reqdoc_scan(directory=01_背景与目标) 扫描提取作引导输入（可重复扫描，不必等下一轮）。init 之后、进入追问前，必须显式向业务提出「投放材料 / 直接口述」的二选一（或问清已投了哪些目录），未得到明确选择不擅自推进追问；业务选直接口述时，先回一句知情确认「那全程来源会是 [问答]，定稿时你需确认『无书面材料』」再继续，部分投放则仅对投放目录扫描、状态条自然显示 [文档]/[问答] 混合、无需该确认。" },
    { id: "reqdoc-r32", stage: "global", text: "已有初稿需求书时，引导业务把初稿放进 00_初稿需求书/ 目录后调用 reqdoc_import(path) 导入（path 可为文件或目录）：工具按路径解析初稿为 [文档] 来源、产出「按机构规约的初评」（逐份规约列 满足/缺失/矛盾），并停在起点等待业务看初评后再逐阶段走（不自动 approve 任何阶段、不替业务快进）。初评后按三类路径补全：① AI 可直接改的结构/术语类（如术语未引原文、章节错序）；② 依赖外部依据（制度原文、系统接口、敏感字段清单）须投放 03_制度与合规 / 04_角色与权限 / 05_系统现状与能力 后 reqdoc_scan 提取；③ 仅业务知晓（真实异常实例、量化指标、权限边界）走 edge 探针追问（A/B/C + 默认推荐，≤3 轮）。初稿定位为「待完善的原始稿」，导入即诊断、不改写初稿本身。" },
    // ---- rules 流程与规则 ----
    { id: "reqdoc-r9", stage: "rules", text: "引导补全主流程：用户输入哪些信息、系统处理后给什么结果；将自然语言转化为字段定义（数据项 / 是否必填 / 校验规则）。" },
    { id: "reqdoc-r10", stage: "rules", text: "自动推演主流程后，在对话里用**可读的纯文本步骤（编号列表 ①→②→③ 或箭头串）**向业务展示确认——CLI/终端不渲染 Mermaid，对话内不得只给 Mermaid 图、也不要裸写 flowchart TD；prd 阶段渲染时，流程图直接内嵌 PRD 正文「5.k.2.13 流程图」（Mermaid 代码块），不再单独落盘附_流程图/（仅当流程图超长/超复杂时才拆分独立文件并引用）。业务说资料已放好则调用 reqdoc_scan(directory=02_流程与数据) 扫描提取字段与流程作输入；综合扫描材料与问答生成数据字典与库表设计（数据实体/字段/主外键关系/校验规则），向业务展示确认。" },
    // ---- edge 边界与异常（最关键）----
    { id: "reqdoc-r11", stage: "edge", text: `按探针清单推进追问（清单与 reqdoc_probe 工具描述同源，每维映射打分卡扣分项）：\n${reqdocProbeRubric()}\nmain_flow/exception 探针须「先要真实案例再答题」——业务给不出实例则明示缺真实素材、卡住提示投放 01~05，不往下走。逐轮追问最多 5 问（见 r2，带 A/B/C 与【默认推荐项】），每轮结束调用 reqdoc_probe(asked=本轮新问探针, gaps=仍缺口探针, round=轮次, business_default=本轮是否全选默认) 记录覆盖；追问最多 3 轮，3 轮后仍未澄清项标 [缺省] 停止追问；到 3 轮上限时，先把未澄清探针逐条列出（附对应打分卡维度与将扣分数），并说明业务的可选项——补充材料/口述补全这些项（我据此扫描或追问补漏）、确认接受 [缺省] 留白（对应维度将扣分）、或开新会话继续；无需一次补全，定稿前随时可补——再进下一环节。` },
    ...
    { id: "reqdoc-r22", stage: "edge", text: "探针覆盖度（进入 prd 硬前置 + 缺口一致校验）：进入 prd 前必须已调用 reqdoc_probe 记录过探针（P0.1 强制前置，workflow_advance(enter prd) 未记录即拦截；材料已全覆盖无追问时可记录一次，asked/gaps 可为空）；一旦记录，服务端校验缺口与打分一致——缺口探针对应打分卡维度不得打满分（缺口+满分=自评不诚实，workflow_advance 进 prd 与 review_submit 会被拒绝）。建议每轮追问结束调用 reqdoc_probe 记录（覆盖度在状态条可见，帮助自评一致）。" },
    ...
    { id: "reqdoc-r14", stage: "prd", text: "按《业务需求说明书》模板**分段渲染**最终 PRD（模板：docs/reqdoc-prd-template.md；模板全文由插件在 prd 阶段自动注入系统提示，见「模板全文」段；以注入的模板全文为唯一依据，渲染须严格逐字遵循，见 reqdoc-r20）。\n\n**分段渲染流程（避免单次输出过长导致 SSE 超时）**：\n\n**第 1 步：生成 PRD 框架**。用 write 工具一次性写入骨架 PRD（写入 07_需求规格产出/N_名称/PRD.md），内容包含：封面（项目信息表、文档变更过程表）→ 第三章 需求概述（3.1~3.6）→ 第四章 术语定义与业务规则（4.1~4.2）→ 第五章 需求功能详述**仅写章节标题「## 第五章 需求功能详述」+ 编号规则说明，功能点块留空**→ 第六章 非功能需求（6.1~6.4）→ 第七章 验收标准（7.1~7.2）。项目信息表与文档变更过程留空占位。\n\n**第 2 步：逐功能点填充第五章**。对每个已确认功能点（k=1,2,...N），循环执行：\n① reqdoc_extract_feature(source=PRD路径, feature=k) 提取当前块内容\n② 读取 06_功能点/k_名称/ 来源摘录 + 问答记录，按模板规则重写该功能点块（14 个固定子小节：5.k.1.1 简要概述~5.k.2.13 流程图，编号全局连续），逐字段标来源 [文档]/[问答]/[缺省]\n③ reqdoc_replace_feature(source=PRD路径, feature=k, newBlock=新内容) 写回（结构不合规则拒绝写入）\n④ reqdoc_check(source=PRD路径, feature=k) 单点校验（可选）\n\n**第 3 步：全篇校验**。所有功能点写完后调用 reqdoc_check(source=PRD路径) 校验整篇。\n\n每功能点内容从 06_功能点/N_名称/ 子目录的来源摘录 + 问答补全，绝不杜撰事实；[缺省] 须写成 [缺省：不适用理由]，禁止裸 [缺省]；[问答] 来源的口语须提炼为规范书面语，不得原话照搬。产出归档：需求澄清记录、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑、最终 PRD 一律写入 07_需求规格产出 目录；PRD 定稿后调用 reqdoc_export(source=PRD 路径) 生成 Word 交付件。" },
    { id: "reqdoc-r20", stage: "prd", text: "渲染铁律 + 字段映射（模板权威约束）：模板全文已由插件注入对话（见系统提示「模板全文」段，无需自行读文件），以注入的模板全文为唯一依据，渲染严格逐字遵循、不调整章节顺序/标题/字段名；如发现模板结构问题如实上报、不擅自修正（归行方模板主管部门）。打分卡扣分项按以下映射落位到模板既有字段：脱敏规则（手机号/身份证遮罩）→功能点 5.k.2.8 交易安全性/5.k.2.9 数据存贮和清理；资金或高危变更留痕与双人复核→5.k.1.2 控制要求/5.k.2.8 交易安全性；总/分/支行数据边界与岗位权限→5.k.1.2 控制要求/5.k.2.1 输入要素的检查；异常边界（网络超时/操作失败/并发重复提交/逆向撤销驳回）→5.k.2.3 异常处理要求/5.k.2.6 清算处理/5.k.2.7 差错处理；接口与数据源（外部系统对接方式/数据来源系统字段）→5.k.2.11 接口与数据源（material 维度：真实接口/字段证据，缺则对应维度扣分）；权限与最小授权（谁可办、可看哪些数据、敏感字段脱敏处理）→5.k.2.12 权限与最小授权（authority 数据边界/岗位权限 + compliance 合规）；流程图→5.k.2.13 流程图（涉及 3 步以上主流程或多角色交互时须内嵌 Mermaid 代码块）；模板确无对应字段的补充内容→5.k.2.2 系统处理过程或功能点描述，来源标注注明「补」。[缺省] 须写成 [缺省：不适用理由]（如 [缺省：本次无外部系统对接]），禁止裸 [缺省]（裸 [缺省] 触发定稿完整性门禁，review_submit 会拦截）。模板外成果（UAT 验收测试用例、低保真界面说明、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑）不插入模板正文，用 write 写入 07_需求规格产出 下子目录（测试用例/、界面草图/、数据字典与库表设计/、权限矩阵与审批流/），并在对应功能点「5.k.2.10 附件」列出清单与相对路径。" },
    { id: "reqdoc-r23", stage: "prd", text: `渲染结构校验：渲染完成并写入 07_需求规格产出 后，调用 reqdoc_check(source=PRD md 相对项目根路径) 对照模板结构 schema 做渲染 diff 校验（章节齐全/顺序/功能点块数/必填字段来源标注）：\n${renderCheckRubric()}\n校验有违规（缺章节/乱序/功能点块数不符/映射字段漏标来源）须修正后重调 reqdoc_check 复查；结构合规后再 review_submit 定稿。` },
    ...
    { id: "reqdoc-r31", stage: "prd", text: "字段定义环节（数据字典，进 prd 硬前置）：渲染前对每个功能点的输入字段逐一定义——字段名、类型、长度/精度、是否必填、取值域/约束、来源系统/接口——并调用 reqdoc_field_dict(fields=[...]) 记录，写入 07_需求规格产出/数据字典与库表设计/数据字典.md。逐字段与业务确认（名称/类型/长度/必填/取值/来源系统），业务确认后才落库；字段定义是 material 维度（真实字段/接口证据）的直接来源，缺失则对应维度扣分。workflow_advance(enter prd) 与 review_submit 两处强制要求 fieldDict 非空（需求确无结构化字段可 skip_field_dict=true 豁免）。已确认功能点较多时可分批提交 reqdoc_field_dict，服务端按 feature 合并。" },
