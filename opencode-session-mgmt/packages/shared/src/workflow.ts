...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r14", stage: "prd", text: "按《业务需求说明书》模板**分段渲染**最终 PRD（模板为权威源，逐字落实由服务端 reqdoc_render_skeleton 生成骨架保证；系统提示「渲染目标结构」段给出结构摘要，渲染须严格逐字遵循，见 reqdoc-r20）。\n\n**分段渲染流程（服务端组装骨架 + 逐小节填充，避免单次输出过长导致截断/SSE 超时）**：\n\n**第 1 步：生成骨架**。调用 reqdoc_render_skeleton(source=07_需求规格产出/N_名称/PRD.md)，由服务端按模板逐字生成章节 1~7 骨架（第五章含已确认功能点的空块）。不要用 write 手写整篇骨架。\n\n**第 2 步：逐小节填充**。用 reqdoc_patch(source=PRD路径, target=小节编号, content=内容, source_tag=标签, reason=缺省理由) 小步写入，每次只填 1~3 个小节：① 第三章 3.1~3.6、第四章 4.1/4.2、第六章 6.1~6.4、第七章 7.1/7.2；② 逐功能点 k 填 5.k.1.1 简要概述 ~ 5.k.2.13 流程图（编号全局连续，内容取自 06_功能点/k_名称/ 来源摘录 + 问答记录）。映射字段小节（5.k.1.2、5.k.2.1/.3/.6/.7/.8/.9/.11/.12）必填 source_tag（\"[文档]\"/\"[问答]\"/\"[缺省]\"），[缺省] 须附 reason；[问答] 来源的口语须提炼为规范书面语，不得原话照搬；绝不杜撰事实。服务端将规范标签写入标题行（如「5.k.2.3 异常处理要求 [文档]」或「5.k.2.6 清算处理 [缺省：本次无清算处理]」），content 不含标签。\n\n**第 3 步：全篇校验**。全部小节填完后调用 reqdoc_check(source=PRD路径) 校验整篇，有违规用 reqdoc_patch 修正后复查。\n\n产出归档：需求澄清记录、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑、最终 PRD 一律写入 07_需求规格产出 目录；PRD 定稿后调用 reqdoc_export(source=PRD 路径) 生成 Word 交付件。" },
    { id: "reqdoc-r20", stage: "prd", text: "渲染铁律 + 字段映射（模板权威约束）：模板由服务端在 reqdoc_render_skeleton 生成骨架时逐字落实（系统提示「渲染目标结构」段给出结构摘要，无需自行读文件），渲染严格逐字遵循、不调整章节顺序/标题/字段名；如发现模板结构问题如实上报、不擅自修正（归行方模板主管部门）。打分卡扣分项按以下映射落位到模板既有字段：脱敏规则（手机号/身份证遮罩）→功能点 5.k.2.8 交易安全性/5.k.2.9 数据存贮和清理；资金或高危变更留痕与双人复核→5.k.1.2 控制要求/5.k.2.8 交易安全性；总/分/支行数据边界与岗位权限→5.k.1.2 控制要求/5.k.2.1 输入要素的检查；异常边界（网络超时/操作失败/并发重复提交/逆向撤销驳回）→5.k.2.3 异常处理要求/5.k.2.6 清算处理/5.k.2.7 差错处理；接口与数据源（外部系统对接方式/数据来源系统字段）→5.k.2.11 接口与数据源（material 维度：真实接口/字段证据，缺则对应维度扣分）；权限与最小授权（谁可办、可看哪些数据、敏感字段脱敏处理）→5.k.2.12 权限与最小授权（authority 数据边界/岗位权限 + compliance 合规）；流程图→5.k.2.13 流程图（涉及 3 步以上主流程或多角色交互时须内嵌 Mermaid 代码块）；模板确无对应字段的补充内容→5.k.2.2 系统处理过程或功能点描述，来源标注注明「补」。映射字段小节的来源标签由 reqdoc_patch 的 source_tag 参数写入标题行（服务端规范写入），[缺省] 须附 reason 参数，禁止裸 [缺省]（裸 [缺省] 触发定稿完整性门禁，review_submit 会拦截）。模板外成果（UAT 验收测试用例、低保真界面说明、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑）不插入模板正文，用 write 写入 07_需求规格产出 下子目录（测试用例/、界面草图/、数据字典与库表设计/、权限矩阵与审批流/），并在对应功能点「5.k.2.10 附件」列出清单与相对路径。" },
