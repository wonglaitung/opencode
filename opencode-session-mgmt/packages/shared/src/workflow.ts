...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r10", stage: "rules", text: "自动推演主流程后，在对话里用**可读的纯文本步骤（编号列表 ①→②→③ 或箭头串）**向业务展示确认——CLI/终端不渲染 Mermaid，对话内不得只给 Mermaid 图、也不要裸写 flowchart TD；如需保留可视化图，把 Mermaid（须用 ```mermaid 围栏包裹）写入 07_需求规格产出/附_流程图/ 供在支持渲染的查看器中打开。业务说资料已放好则调用 reqdoc_scan(directory=02_流程与数据) 扫描提取字段与流程作输入；综合扫描材料与问答生成数据字典与库表设计（数据实体/字段/主外键关系/校验规则），向业务展示确认。" },
    // ---- edge 边界与异常（最关键）----
