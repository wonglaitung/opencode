...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r14", stage: "prd", text: "按《业务需求说明书》模板渲染最终 PRD（模板：docs/reqdoc-prd-template.md；模板全文由插件在 prd 阶段自动注入系统提示，见「模板全文」段；以注入的模板全文为唯一依据，渲染须严格逐字遵循，见 reqdoc-r20；仅当插件找不到模板文件时才按内联骨架渲染）：封面（项目信息表、文档变更过程表）→ 第一章 需求概述（需求类型：新增/更改、流程优化/跨部门/总行开发、希望完成时间、提出原因及功能概述）→ 第二章 术语定义与业务规则（术语定义、业务规则）→ 第三章 需求功能详述（按已确认功能点：编号/名称/优先级，输入要素：简要概述/控制要求，处理要求：输入要素检查/系统处理过程/异常处理/提示信息/其他要求/清算处理/差错处理/交易安全性/数据存贮和清理/附件）。每功能点内容从 05_功能点/N_名称/ 子目录的来源摘录 + 问答补全，逐字段标来源：文档提取标 [文档]、问答补全标 [问答]、尚未获得的信息留白并标 [缺省]，绝不杜撰；未涉及项在 ○/● 中选「不涉及/不适用」并留白正文；项目信息表与文档变更过程属项目元数据，不主动问业务，渲染时留空占位。产出归档：需求澄清记录、自动提取的 Mermaid 流程图、数据字典与库表设计、RBAC 权限控制矩阵与审批流控制逻辑、最终 PRD 一律写入 06_需求规格产出 目录；PRD 定稿后调用 reqdoc_export(source=PRD 路径) 生成 Word 版（.docx）交付件，与 md 同目录归档。" },
    ...
    { id: "reqdoc-r23", stage: "prd", text: `渲染校验（质量飞轮 P2）：渲染完成并写入 06_需求规格产出 后，调用 reqdoc_check(source=PRD md 相对项目根路径) 对照模板结构 schema 做渲染 diff 校验（章节齐全/顺序/功能点块数/必填字段来源标注）：\n${renderCheckRubric()}\n校验有违规（缺章节/乱序/功能点块数不符/映射字段漏标来源）须修正后重调 reqdoc_check 复查；结构合规后再 review_submit 定稿。` },
    { id: "reqdoc-r24", stage: "prd", text: "渲染门禁（柔性 + 定稿复核）：reqdoc_check 不强制调用（未记录则 review_submit 放行，靠打分卡 ≥85 与产出度量兜底）；一旦调用即记录，review_submit 定稿时会重读源 md 复核——结构违规（缺章节/乱序/功能点块数不符/映射字段漏标来源）与 [缺省] 字段对应打分卡维度打满分都会被拒绝。建议渲染后都调用 reqdoc_check 自查（校验结果在状态条可见），缺料字段如实标 [缺省] 并在 reqdoc_score 中对应维度扣分。" },
    // ---- review 业务确认（核心）----
