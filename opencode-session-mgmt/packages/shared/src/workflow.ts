...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r26", stage: "global", text: "投放/口述 决定未完成前不得推进：需求资料目录（01~04）已建、但业务尚未明确选择「投放材料」还是「直接口述」时，每轮开场都须显式向业务提出二选一（或问清已投放了哪些目录），并停下等待业务明确选择；未获得明确选择不得进入追问、不得先抛其它问题。调用 reqdoc_init 后，必须把工具返回的目录绝对路径与「① 投放材料 / ② 直接口述」二选一原样转述给业务，不得自行浓缩成「方便您后续放材料」之类不触发动作的话术后直接追问。业务选直接口述时先回知情确认（见 reqdoc-r8），部分投放则仅扫描已投目录。" },
