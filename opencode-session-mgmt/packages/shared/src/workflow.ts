...
export const REQDOC: WorkflowDefinition = {
  ...
  rules: [
    ...
    { id: "reqdoc-r10", stage: "rules", text: "自动推演 Mermaid 流程图，反向展示给业务确认；**输出任何 Mermaid 图都必须用围栏代码块包裹，且语言标识须写 mermaid（即以 ```mermaid 起、以 ``` 止），否则前端不渲染、只显示源码；严禁裸写 flowchart TD / graph TD 这类图声明行**；业务说资料已放好则调用 reqdoc_scan(directory=03_流程与数据) 扫描提取字段与流程作输入；综合扫描材料与问答生成数据字典与库表设计（数据实体/字段/主外键关系/校验规则），向业务展示确认。" },
    // ---- edge 边界与异常（最关键）----
