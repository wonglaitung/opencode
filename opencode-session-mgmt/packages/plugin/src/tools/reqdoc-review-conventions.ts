...
export const REQDOC_CONVENTION_REVIEW_PROMPT = `请按以下 7 项检查标准，对刚导入的初稿逐条点评，每条给出「满足 / 缺失 / 矛盾」结论并引用初稿具体段落：
1. 范围与边界：需求范围与边界是否清晰，是否点明「不在范围内」的内容。
2. 术语与命名：关键术语是否定义、是否引用制度/行内原文（而非自造表述）。
3. 状态与生命周期：关键业务对象的状态与生命周期是否定义（如待处理/已处理/已撤销）。
4. 异常分类：三类异常是否覆盖——业务异常（如重复提交、权限不足）、系统异常（如网络中断、服务不可用）、边界异常（如超时、数据格式错误）。
5. 性能与安全指标：性能与容量、可用性与可靠性、安全与合规是否给出量化指标。
6. 系统现状与能力：外部系统对接方式、数据来源系统/字段是否逐功能点说明。
7. 可验证性与验收：验收标准是否可测、可量化（功能点验收指标 + 量化验收口径）。

对每条「缺失 / 矛盾」，须标注补全路径（三选一）：
- AI 修：结构/术语类（术语未引原文、章节错序、命名不规范）→ 由 AI 直接改，不需业务额外提供。
- 人工补材料：依赖外部依据（制度原文、系统接口、敏感字段清单、现有系统能力）→ 请业务将材料放入 03_制度与合规 / 04_角色与权限 / 05_系统现状与能力 对应目录后告诉我扫描。
- 对话补：仅业务知晓（真实异常实例、量化指标、权限边界）→ 通过后续对话追问补全。

输出格式（建议 Markdown 表格）：| 检查项 | 结论 | 引用段落 | 建议补全路径 |。初评是诊断，不改写初稿本身；待业务确认后逐阶段走工作流补全。`
