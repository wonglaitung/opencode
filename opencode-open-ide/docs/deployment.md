...
| win32 打开 IDE 无反应（工具返回成功但 IDE 没起来） | 两类根因均已修复：① `where code` 会同时返回无后缀的 POSIX sh 脚本（`...\bin\code`，供 WSL/linux）与真正的 Windows shim（`code.cmd`）；旧版取第一行选到 sh 脚本，cmd.exe 无法执行——已按扩展名优先 `.exe`/`.cmd`/`.bat` 挑选（`pickWindowsExecutable`）。② `shell:true` 下 binary 含空格（如 `...\Microsoft VS Code\bin\code.cmd`）未加引号被 cmd.exe 拆词（'Microsoft' is not recognized）——已由 `buildSpawnCommand` 对 binary 与参数统一加引号。升级插件后复测即可 |
