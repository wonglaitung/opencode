...
- **启动即忘**：`spawn` 带 `detached + stdio:ignore + unref()`，daemon 不挂起；**dispose 不杀 IDE**（IDE 由用户自主关闭，区别于 Edge 调试实例）。win32 `shell:true` 下 **binary 与参数都须经 `buildSpawnCommand` 加引号**——只引参数不引 binary，含空格的 binary 路径（如 `...\Microsoft VS Code\bin\code.cmd`）被 cmd.exe 拆词致静默失败（曾致 VS Code 不启动）。
